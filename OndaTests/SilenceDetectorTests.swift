...
final class SilenceDetectorTests: XCTestCase {
    ...
    func test_realSpeechResets_allowsNextSkip() {
        var d = SilenceDetector()
        var count = 0
        for _ in 0 ..< 10 where d.consume(rms: 0.001, bufferSeconds: 0.1) != nil { count += 1 }
    ...
    func test_briefBlip_doesNotResetQuietRun() {
        ...
        var d = SilenceDetector()
        var skips = 0
        for _ in 0 ..< 4 where d.consume(rms: 0.001, bufferSeconds: 0.1) != nil { skips += 1 }  // 0.4s quiet
        _ = d.consume(rms: 0.5, bufferSeconds: 0.05)                                          // 50ms blip
        for _ in 0 ..< 3 where d.consume(rms: 0.001, bufferSeconds: 0.1) != nil { skips += 1 }  // 0.3s quiet
